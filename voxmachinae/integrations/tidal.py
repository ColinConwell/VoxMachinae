"""TIDAL OAuth integration client."""
...
class TidalIntegration(MusicIntegrationProvider):
    provider_name = "tidal"
    authorize_url = "https://login.tidal.com/authorize"
    token_url = "https://oauth2.tidal.com/v1/oauth2/token"

    def __init__(self, client_id: str, client_secret: str, scopes: list[str] | None = None) -> None:
        if not client_id or not client_secret:
            raise ValueError("TIDAL client credentials are required")
        self.client_id = client_id
        self.client_secret = client_secret
        self.scopes = scopes or ["r_usr", "w_usr"]
