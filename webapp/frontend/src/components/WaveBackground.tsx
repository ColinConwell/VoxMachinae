  ...
  useEffect(() => {
    ...
    function getWaveHeight(gx: number, gz: number, time: number): number {
      ...
      for (const w of waves) {
        y += w.amp * Math.sin(gx * w.freqX + w.phaseX + time * w.speed * 1000)
                    * Math.cos(gz * w.freqZ + w.phaseZ + time * w.speed * 700)
      }
      // Add a subtle radial ripple
      const dist = Math.sqrt(gx * gx + gz * gz)
      y += 12 * Math.sin(dist * 0.04 - time * 0.0006 * 1000)
      return y
    }
